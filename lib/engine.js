...
module.exports = function (s, conf) {
    ...
    s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
      ...
      getQuote(function (err, quote) {
        ...
        if (!s.start_capital) {
          ...
          if (so.mode !== 'sim') {
            pushMessage(`[${s.exchange.name}.${s.asset}-${s.currency}]`, '💵 balance ' + s.real_capital + ' ' + s.currency  + '\n')
          }
        } else {
    ...
    let cb = function (err, order) {
      ...
      if (_cb)
        ...
        if (err.message.match(nice_errors)) {
          if (!so.silent) console.error((err.message + ': ' + err.desc).red)
        } else {
          memDump()
          console.error('\n')
          console.error(err)
  ...
  function executeOrder (order, trade_type) {
    ...
    if (so.stats) {
      let trade_emoji
      let execution_time = moment.duration(my_trade.execution_time).as('seconds')
      ...
      if (trade_type === 'buy') trade_emoji = '📈'
      if (trade_type === 'sell') trade_emoji = '📉'
      let order_complete = `\n${trade_type} order completed at ${completion_time}:\n` +
          `${asset_qty} at ${currency_price}\n` +
          `total ${total_price}\n` +
          `${slippage} slippage (orig. price ${orig_price})\n` +
          `execution: ${n(execution_time).format('0.0')} seconds\n`
      console.log((order_complete).cyan)
      balanceFigure(function (err, balance) {
        if (err) {
          if (err.desc) console.error(err.desc)
          if (err.body) console.error(err.body)
          throw err
        }
        let order_complete_notifier = `${trade_emoji} ${trade_type} order completed at ${completion_time}\n` +
            `${asset_qty} at ${currency_price}, total ${total_price}\n` +
            `${slippage} slippage (orig. price ${orig_price})\n` +
            `execution: ${n(execution_time).format('0.0')} seconds\n` +
            `balance: ${n(balance).format('0.000000')} ${s.currency}\n`
        pushMessage(`[${s.exchange.name}.${s.asset}-${s.currency}]`, order_complete_notifier)
      })
    }
